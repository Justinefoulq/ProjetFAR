...
void *fctT3(void* input){
	...
	struct sockaddr_in aC =((struct SocketServeur*)input)->aC;
	//On recoit le nom du fichier
	recvfrom(dS3,nom,sizeof(nom),0, (struct sockaddr*) &aC, &lg);
	...
	recvfrom(dS3,(char*) &nbR,sizeof(int),0, (struct sockaddr*) &aC, &lg);
	//printf("La taille est %d\n",nbR);
	for(i=0; i<(nbR);i++){
		recvfrom(dS3,msg,sizeof(msg),0, (struct sockaddr*) &aC, &lg);
		fputs(msg,f);
	}
	fclose(f);
...
void *fctT4(void* input){
	...
	if (fps == NULL){
		...
		sendto(dS4,nom,strlen(nom)+1,0, (struct sockaddr*) &aC, lg);
		//printf("La taille est %d\n",cpt);
		fseek(fps,0, SEEK_SET);
		sendto(dS4,(char*) &cpt,sizeof(int),0, (struct sockaddr*) &aC, lg);
		// Lire et afficher le contenu du fichier
		while (fgets(str, 1000, fps) != NULL) {
			//fprintf(fp1,"%s",str);
			sendto(dS4,str,strlen(str)+1,0, (struct sockaddr*) &aC, lg);
		}
...
void *fctT1(void* input){
	...
	struct sockaddr_in aC = SS->aC;

	//Tan que le message n'est pas fin
	while(socketferme){
		recvfrom(SS->dSock,msg,sizeof(msg),0, (struct sockaddr*) &aC, &lg);
		//Si le message est le message de fin
		if (!strcmp(msg,messfin)){
			socketferme=0;
			sendto(SS->dSock,messfin,strlen(messfin)+1,0, (struct sockaddr*) &aC, lg);
...
void *fctT2(void* input){
	...
	while(socketferme){
		...
		sendto(SS->dSock,msg,strlen(msg)+1,0, (struct sockaddr*) &aC, lg);
...
int main(int argc, char ** argv){
	...
	printf("Bienvenu sur le tchat\n");

	//Envoie du pseudo
	printf("Entrer votre pseudo :");
	fgets(pseudo1,15,stdin);
	sendto(dSock, pseudo1, strlen(pseudo1)+1, 0, (struct sockaddr*) &adServ, lg);
	
	//Reception ad de l'autree client
	recvfrom(dSock,&aC,sizeof(struct sockaddr_in),0, (struct sockaddr*) &adServ, &lg);

	//Reception pseudo autre client 
	recvfrom(dSock,&pseudo2,sizeof(pseudo2),0, (struct sockaddr*) &adServ, &lg);
